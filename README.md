# Monad Master Mind

## Overview
Master Mind – on-chain rock-paper-scissors money game on Monad, combining a backend API, a frontend app, and a Solidity smart contract.

## Tech stack
- **Monad (EVM-compatible)** with a Solidity Master Mind contract.
- **Backend:** Node.js, Fastify, ethers.
- **Frontend:** Next.js, React, wagmi, TailwindCSS (or similar styling utilities).

## Project structure
- `backend/` – API server that connects to the Master Mind contract on Monad.
- `frontend/` – Next.js app that provides the lobby, battle, profile, and leaderboard experiences.
- `contracts/` – Solidity Master Mind smart contract (if present in the repository).

## Prerequisites
- Node.js (LTS) with npm or pnpm.
- Access to a Monad RPC endpoint (testnet or mainnet).
- A deployed Master Mind contract address on the same network.

## Environment variables
### Backend
- `MONAD_RPC_URL` – RPC endpoint URL.
- `CONTRACT_ADDRESS` – Master Mind contract address on Monad.
- `PORT` – backend port (default `3001`).
- `CORS_ORIGIN` – frontend origin allowed by CORS (e.g. `http://localhost:3000`).

### Frontend
- `NEXT_PUBLIC_BACKEND_URL` – URL of the backend API (e.g. `http://localhost:3001`).
- `NEXT_PUBLIC_CHAIN_ID` – chain id for the Monad network.
- `NEXT_PUBLIC_RPC_URL` – RPC endpoint used by wagmi/wallet connections.
- `NEXT_PUBLIC_MONAD_RPC_URL` – optional alias for the RPC URL used in the current frontend code.

## How to run locally
1. Clone the repository.
2. Install dependencies in both `backend/` and `frontend/` (e.g. `npm install` or `pnpm install`).
3. Copy the provided `.env.example` files to real `.env` / `.env.local` files and fill in real values for your Monad RPC URL and Master Mind contract address.
4. Start the backend (from `backend/`): `npm run dev`.
5. Start the frontend (from `frontend/`): `npm run dev`.
6. Open the app in your browser at `http://localhost:3000`.

The app expects a live Monad RPC URL and a valid Master Mind contract address to function correctly.

## Quick Start
Run both backend and frontend together from the repository root:
1. Install dependencies in `backend/` and `frontend/` (e.g. run `npm install` in each directory).
2. Install root-level tooling (`npm install` at the repo root) to pull in `concurrently` for the combined runner.
3. Copy environment templates (`backend/.env.example` and `frontend/.env.local.example`) to their runtime files with your Monad RPC URL and Master Mind contract address.
4. From the repository root, start both servers with `npm run dev:all` (backend on port `3001`, frontend on port `3000`).
5. Open `http://localhost:3000` while both services run side by side.

Quick Start

npm install # installs root dependencies + concurrently
npm run dev:all # starts backend + frontend concurrently

Backend runs at: http://localhost:4000
Frontend runs at: http://localhost:3000

## Local development checklist
1. Copy `backend/.env.example` to `backend/.env` and `frontend/.env.local.example` to `frontend/.env.local`, then fill in:
   - `MONAD_RPC_URL`
   - `CONTRACT_ADDRESS`
   - `NEXT_PUBLIC_BACKEND_URL`
   - `NEXT_PUBLIC_RPC_URL` / `NEXT_PUBLIC_MONAD_RPC_URL`
2. From the repository root, run `npm install` once (if not already done) to install root tools.
3. In the backend and frontend folders, ensure dependencies are installed (`npm install` in each, if not already).
4. Start both services with `npm run dev:all` from the repo root.
5. Visit `http://localhost:3000/dev/status` in the browser and confirm:
   - Backend health is reported as OK.
   - The RPC URL and contract address values are populated as expected.
6. If anything fails, re-check environment variables and that ports 3000 and 4000 are free.
